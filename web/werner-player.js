...
function playSlide(element, slideConfigs) {
	(0, _preact.render)((0, _preact.h)(_player2.default, { configs: slideConfigs, container: element }), element);
}
...
var SlideItem = function (_Component) {
	...
	function SlideItem(props) {
		...
		_this.injectScript = function () {
			...
			_this.layoutInterval = setInterval(function () {
				return _this.initializeHypeLayout();
			}, 1000);
		};
		...
		_this.initializeHypeLayout = function () {
			...
			var _this$scriptContainer = _this.scriptContainer,
			    ...
			    clientWidth = _this$scriptContainer.clientWidth,
			    hypeDocuments = global.HYPE.documents,
			    extractedName = extractHypeNameFromUrl(_this.props.url),
			    hypeInstance = hypeDocuments[extractedName] || hypeDocuments[_this.props.name];
			...
			var setupLayout = function setupLayout() {
				...
				if (_this.props.onHypeLayout) _this.props.onHypeLayout({
					sceneWidth: currentScene.width,
					sceneHeight: currentScene.height
				});

				_this.setState({ height: currentScene.height * widthRatio });
...
function extractHypeNameFromUrl(url) {
	var endIndex = url.indexOf('.hyperesources/'),
